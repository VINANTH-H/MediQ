...
var VoyageEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	...
	*/
	constructor(fields) {
		...
		const apiKey = fieldsWithDefaults?.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("VOYAGE_API_KEY") ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("VOYAGEAI_API_KEY");
	...
	async embeddingWithRetry(request) {
		const makeCompletionRequest = async () => {
			...
			if (!response.ok) {
				...
				const err = /* @__PURE__ */ new Error(`Voyage AI API error (HTTP ${response.status}): ${message}`);
