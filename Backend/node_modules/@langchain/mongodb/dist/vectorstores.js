...
var MongoDBAtlasVectorSearch = class MongoDBAtlasVectorSearch extends VectorStore {
	...
	*/
	async similaritySearchWithScore(query, k = 4, filter = void 0) {
		if (this.useAutoEmbedding) {
			if (this.rerankOptions) return this.textBasedRerankSearchWithScore(query, k, filter);
			return this.textBasedSearchWithScore(query, k, filter);
		}
		if (this.rerankOptions) return this.rerankSearchWithScore(query, k, filter);
	...
	*/
	...
	async textBasedRerankSearchWithScore(query, k, filter) {
	...
	async rerankSearchWithScore(query, k, filter) {
		const { model, path } = this.rerankOptions;
		const rerankPath = path ?? this.textKey;
		const queryEmbedding = await this.embeddings.embedQuery(query);
		const postFilterPipeline = filter?.postFilterPipeline ?? [];
